...
function scheduleDelayedRestartAfterFlood(authMode: 'qr' | 'code'): void {
  ...
  console.log(chalk.yellow(`⏳ Esperando ${FORCE_RESTART_WAIT_MS / 1000}s antes de reintentar...`));
  setTimeout(() => {
    restartCount = 0;
    firstRestartTime = null;
    isAuthenticated = false;
...
async function main(): Promise<void> {
  ...
  if (cliAuthMode === 'qr') {
    ...
    console.log(chalk.cyan('Usando método: ') + chalk.bold.green('Código de Pareamiento'));
  } else if (cliAuthMode) {
    console.log(
      chalk.red(
    ...
    console.log(chalk.yellow('⚡ Sesión existente detectada, arrancando directamente...\n'));
    selectedAuthMode = 'qr';
  } else {
    if (!existsSync(BOOT_FLAG)) {
      await mostrarBannerVania();
    } else {
      console.log(chalk.yellow('⚡ Detectado arranque previo, saltando animación...\n'));
    }
