...
export class TranslateCommand extends Command {
  ...
  usage =
    "!traducir <idioma> <texto> | responde un mensaje con !traducir <idioma>";
  examples = [
  ...
  async execute(ctx: MessageContext): Promise<void> {
    ...
    try {
      const { translated, detectedLang } = await this.translate(
        textToTranslate,
        targetLang,
      );
      ...
      await ctx.react("❌");
      await ctx.reply(`❌ Error al traducir: ${error.message}`);
    }
  }
}
